...
4. **中间人机制**：如果 P2P 连接失败，则使用种子节点作为中间人，通过种子节点的地址进行数据传输。

5. **数据传输**：
   - 成功连接后，两个节点可以直接使用 `TRANSPORT` 事件发送数据。
   - 如果直连失败，种子节点会中转数据包，确保数据传输成功。
...
4. **传输模式** 成功连接后，两个节点可以直接使用 `TRANSPORT` 事件发送数据。如果直连失败，种子节点会中转数据包，确保数据传输成功。
...
### 地址信息交换
在代码库的 NAT 穿透机制中，通过 `TOUCHREQUEST` 和 `TOUCHED` 事件实现地址信息的交换：
- **`TOUCHREQUEST`**：
  一个节点请求中间种子节点帮助它与目标节点建立连接。
- **`TOUCHED`**：
  中间种子节点回应请求，将目标节点的地址返回给请求方。

这些事件实现了类似 STUN 的功能，帮助两个节点在 NAT 后获取对方的地址。

### 连接建立与确认
- 使用 `CONNECTREQUEST` 和 `CONNECT` 事件完成两节点之间的直接连接。
- 如果连接失败，则继续通过种子节点作为中间人传递消息，类似 TURN 的数据转发模式。

### 数据转发
在 `Transport` 方法中，数据传输采用分片模式。如果两个节点无法直接建立连接，则可以通过种子节点中转数据。这一机制与 TURN 的数据转发方式高度相似。
...
func (peer *Peer_T) Transport(rAddr *net.UDPAddr, data []byte) error {
    ...
    // 通过种子节点中转
    if rAddr == nil {
        // 使用 RemoteSeeds 中的一个地址作为中转目标
        for seed := range peer.RemoteSeeds {
            seedAddr, _ := net.ResolveUDPAddr("udp", seed)
            peer.Conn.WriteToUDP(data, seedAddr)
        }
    } else {
        // 直接传输到目标地址
        peer.Conn.WriteToUDP(data, rAddr)
    }
    return nil
}
...
TURN 的基本原理是通过一个中间服务器（TURN 服务器）实现 NAT 穿透。两个通信的客户端分别与该服务器建立连接，并通过它进行数据转发。核心步骤包括：
1. 客户端与 TURN 服务器建立连接并申请临时的公网地址。
2. 两个客户端使用服务器分配的地址相互通信，服务器负责数据转发。
...
| **特性**           | **TURN**                     | **代码库**                                   |
|--------------------|------------------------------|--------------------------------------------|
| **中间人机制**      | TURN 服务器作为中间人         | `RemoteSeeds` 列表充当中间人角色            |
| **地址分配**        | 通过 TURN 分配公网地址         | `JOIN` 和 `TOUCHREQUEST` 事件交换地址        |
| **数据转发**        | TURN 服务器转发数据           | 中间种子节点或直接连接节点转发数据          |
| **直连优化**        | 优先尝试 P2P 直连，失败时转发   | 同样优先尝试 P2P 直连，无法直连时种子中转    |
| **事件触发机制**    | N/A                          | `JOIN`、`TOUCHREQUEST`、`CONNECT` 等事件驱动 |
...
关于NAT 穿透，`q2p`是通过种子节点列表（`RemoteSeeds`）实现，工作机制与 TURN 十分相似，但更加灵活：

1. 优先尝试直连，减少中转服务器的负载。
2. 在无法直连时，使用中间节点中转数据，保证通信可靠性。
3. 支持事件驱动模式，通过事件简化了地址交换和连接管理逻辑。

这种设计能够适应复杂的 NAT 网络环境，同时兼顾高效性和可靠性。
